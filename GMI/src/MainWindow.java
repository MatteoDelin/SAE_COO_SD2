...
public class MainWindow implements ActionListener {
    ...
    private JMenuItem userCreate;
    private JMenuItem userModify;
    private JMenuItem userDelete;
    private JMenuItem userPrint;
    ...
    public MainWindow() {
        ...
        cTop5.addActionListener(new ActionListener()    { public void actionPerformed(ActionEvent e) { showPanel(new TopRessourcesChart()); } });
        cTopUser.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { showPanel(new TopUserChart()); } });
        cPropDom.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { showPanel(new DureeMoyenneDomaineChart()); } });
        cPeriode.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { showPanel(new TauxEmpruntPeriodeChart()); } });
        cDomaine.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { showPanel(new TauxEmpruntDomaineChart()); } });
        cEvol.addActionListener(new ActionListener()    { public void actionPerformed(ActionEvent e) { showPanel(new EvolutionTauxChart()); } });
    ...
    private void wireCreationRessources(final CreationRessources p) {
        ...
        p.getCreateButton().addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (nom.isEmpty() || nom.equals("Ressource name")) { showError("Please enter a ressource name."); return; }
                if (Ressources.print_user(nom) != null) { showError("Ressource " + nom + " already exists."); return; }
    ...
    private void wireModifieRessources(final ModifieRessources p) {
        ...
        p.getModifyButton().addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                r.setNom(nouveauNom); r.setDescription(nouvelleDesc); r.setDomaine(nouveauDom);
    ...
    private void wireDeleteRessources(final DeleteRessources p) {
        p.getDeleteButton().addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (nom == null) { showError("Select a ressource."); return; }
                int choix = JOptionPane.showConfirmDialog(frame, "Delete ressource \"" + nom + "\" ?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (choix == JOptionPane.YES_OPTION) {
    ...
    private void wirePrintRessources(PrintRessources p) {
        ...
        for (int i = 0; i < Ressources.liste_ressource.size(); i++) {
            ...
            data[i][0] = r.getNom(); data[i][1] = r.getDescription();
            data[i][2] = r.getDomaine(); data[i][3] = r.getLast_maj() != null ? r.getLast_maj().toString() : "";
        }
        p.getTable().setModel(new javax.swing.table.DefaultTableModel(data, colonnes));
    }
    ...
    private void wireCreationReservations(final CreationReservations p) {
        ...
        p.getCreateButton().addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Reservations conflit = Reservations.findConflict(ress, dateD, heure, duree, null);
                if (conflit != null) {
                    showError(String.format(
                        "Conflict: \"%s\" is already reserved by \"%s\" from %s for %d min.",
                        ress.getNom(),
                        conflit.getUser().getNom(),
                        conflit.getHeure().format(DateTimeFormatter.ofPattern("HH:mm")),
                        conflit.getDuree()));
                    return;
                }
                // ────────────────────────────────────────────────────────────
                ...
                new Reservations(user, ress, dateD, heure, duree,
                    typeStr.isEmpty() ? "Emprunt" : typeStr);
    ...
    private void wireModifieReservations(final ModifieReservations p) {
        ...
        p.getModifyButton().addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                // ── Vérification de conflit (en excluant la réservation en cours de modif) ──
                Reservations conflit = Reservations.findConflict(
                    res.getRessource(), res.getDate(), nouvelleHeure, nouvelleDuree, res);
                if (conflit != null) {
                    showError(String.format(
                        "Conflict: \"%s\" is already reserved by \"%s\" from %s for %d min.",
    ...
    private void wirePrintReservations(PrintReservations p) {
        ...
        for (int i = 0; i < Reservations.liste_reservations.size(); i++) {
            ...
            data[i][0] = r.getUser().getNom(); data[i][1] = r.getRessource().getNom();
            data[i][2] = r.getDate()  != null ? r.getDate().toString()  : "";
            data[i][3] = r.getHeure() != null ? r.getHeure().toString() : "";
            data[i][4] = String.valueOf(r.getDuree()); data[i][5] = r.getType_emprunt();
        }
        p.getTable().setModel(new javax.swing.table.DefaultTableModel(data, colonnes));
    }
    ...
    private String[] domainesConnus() {
        ...
        for (int i = 0; i < Ressources.liste_ressource.size(); i++) {
            ...
            d = d.trim();
            boolean found = false;
            for (int j = 0; j < list.size(); j++) { if (list.get(j).equals(d)) { found = true; break; } }
            if (!found) list.add(d);
        }
    ...
    private String[] typesEmpruntConnus() {
        ...
        for (int i = 0; i < Reservations.liste_reservations.size(); i++) {
            ...
            t = t.trim();
            boolean found = false;
            for (int j = 0; j < list.size(); j++) { if (list.get(j).equals(t)) { found = true; break; } }
            if (!found) list.add(t);
        }
        for (int i = 0; i < list.size() - 1; i++)
            for (int j = 0; j < list.size() - 1 - i; j++)
    ...
    private void showInfo(String msg)  { JOptionPane.showMessageDialog(frame, msg, "Success", JOptionPane.INFORMATION_MESSAGE); }
}
